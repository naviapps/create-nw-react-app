...
function cleanup {
  ...
  # rm ./packages/nw-react-scripts/template/src/__snapshots__/App.test.js.snap
...
# We need to install create-nw-react-app deps to test it
cd "$root_path"/packages/create-nw-react-app
npm install
cd "$root_path"
...
then
  cd $temp_app_path
  err_output=`node "$root_path"/packages/create-nw-react-app/index.js test-node-version 2>&1 > /dev/null || echo ''`
...
# ******************************************************************************
# Next, pack nw-react-scripts and create-nw-react-app so we can verify they work.
# ******************************************************************************
...
# Go to nw-react-scripts
cd "$root_path"/packages/nw-react-scripts
...
scripts_path="$root_path"/packages/nw-react-scripts/`npm pack`
...
cd $temp_app_path
create_nw_react_app --scripts-version="$scripts_path" test-app

# ******************************************************************************
# Now that we used create-nw-react-app to create an app depending on nw-react-scripts,
