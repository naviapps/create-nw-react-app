...
function tryGitCommit(appPath) {
  try {
    ...
    execSync(
      'git commit -m "Initialize project using Create NW.js React App"',
      {
        stdio: 'ignore',
      }
    );
...
module.exports = function (
  ...
  if (!templateName) {
    ...
    console.error(
      `You can fix this by running ${chalk.cyan(
        'npm uninstall -g create-nw-react-app'
      )} or ${chalk.cyan(
        'yarn global remove create-nw-react-app'
      )} before using ${chalk.cyan('create-nw-react-app')} again.`
    );
    return;
  }
  ...
  if (templateJson.dependencies || templateJson.scripts) {
    ...
    console.log(
      chalk.red(
        'Root-level `dependencies` and `scripts` keys in `template.json` were deprecated for Create NW.js React App 5.\n' +
